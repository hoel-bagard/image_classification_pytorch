...
def main() -> None:
    ...
    nb_imgs = 10000
    with (data_path / "batches.meta").open("rb") as pickle_file:
        label_map = pickle.load(pickle_file, encoding="bytes")[b"label_names"]

    output_path.mkdir(parents=True, exist_ok=True)
    with (output_path / "classes.names").open("w") as classes_file:
    ...
    for batch_idx, pickle_batch_path in enumerate(data_path.glob("data_*"), start=1):
        ...
        with pickle_batch_path.open("rb") as pickle_file:
    ...
    with (data_path / "test_batch").open("rb") as pickle_file:
